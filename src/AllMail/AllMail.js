...
const AllMail = () => {
  ...
  const newArray = recieveList.map((currentitem) => (
    <Link to={`/inbox/reciever/${RecieverEmail}/${currentitem.id}`} key={currentitem.id}>
      {" "}
      <div  style={{ marginLeft: "40px" }}>
        ...
        <p style={{ display: "inline-flex", margin: "0px 100px 0px 70px",width:"20rem" }}>
          {currentitem.recieverName}
        </p>
        <h4 style={{ display: "inline-flex",width:"50 rem" }}>{currentitem.subject}</h4>
        <hr></hr>
      </div>
    </Link>
  ));
